...
class Client {
  ...
  account={
    getAll:()=>HttpClient.get<AccountType[]>(API_ENDPOINTS.ACCOUNTS),
    create:(input:AccountType)=>HttpClient.post<AccountType>(API_ENDPOINTS.ACCOUNTS,input),
    update:(input:AccountType)=>HttpClient.put<AccountType>(`${API_ENDPOINTS.ACCOUNTS}/${input.id}`,input),
    delete:(id:string)=>HttpClient.delete<boolean>(`${API_ENDPOINTS.ACCOUNTS}/${id}`),
    getOrderStates:(props={})=>HttpClient.get<string[]>(`${API_ENDPOINTS.ORDER_STATES}?${new URLSearchParams(props).toString()}`),
    getPaymentStates:(props={})=>HttpClient.get<string[]>(`${API_ENDPOINTS.PAYMENT_STATES}?${new URLSearchParams(props).toString()}`),
  }
  ...
  orders = {
    ...
    find: (queryKey: QueryParamsType) => {
      ...
      return HttpClient.get<Order>(`${API_ENDPOINTS.ORDER}`, {
        ...params,
        search: HttpClient.formatSearchParams(queryKey.search??{}),
  ...
  withdrawClient = {
    ...
    paginated: ({ shop_id, ...params }: Partial<any>) => {

      return HttpClient.get<any>(API_ENDPOINTS.WITHDRAWS, {
        shop_id,
        searchJoin: 'and',
        ...params,
    ...
    create(data: any) {
      return HttpClient.post<Type>(API_ENDPOINTS.WITHDRAWS, data);
    },
