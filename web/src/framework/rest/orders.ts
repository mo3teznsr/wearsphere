...
export const useOrderStatuses=(props={start:moment().subtract(1,'months').format('YYYY-MM-DD'),end:moment().add(1,'months').format('YYYY-MM-DD')})=>{
  const { data, isLoading, error } = useQuery<any[], Error>(
    [],
    () => client.account.getOrderStates(props),
    {
      keepPreviousData: true,
    }
  );
