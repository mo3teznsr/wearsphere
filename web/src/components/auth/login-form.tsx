...
const LoginForm: React.FC<Props> = ({ layout = 'modal' }) => {
  ...
  return (
    <div className='flex gap-4 flex-row'>
      ...
      <div
			className=" hidden lg:flex w-full flex-2 rounded-3xl bg-no-repeat bg-center bg-cover"
			style={{
				backgroundImage: "url(/assets/images/cover.avif)",
			}}
		>
      
      </div>
      <div className='flex-1'>
    <div className="w-full px-5 py-5 mx-auto overflow-hidden bg-white border border-gray-300 rounded-xl sm:w-96 md:w-450px sm:px-8">
      ...
      <div className="grid grid-cols-1 gap-4 mt-2">
        ...
        {/* <Button
          disabled={loading}
          className="h-11 md:h-12 w-full mt-2.5 bg-slate-100 text-slate-800 hover:bg-slate-200 hover:text-slate-900"
          onClick={() => signIn('google')}
        >
          <IconBrandGoogle strokeWidth={1} className="text-sm sm:text-base ltr:mr-1.5 rtl:ml-1.5" />
