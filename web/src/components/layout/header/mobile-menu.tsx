    ...
    <>
      <div className="flex flex-col justify-between w-full h-full">
        ...
        <Scrollbar className="menu-scrollbar flex-grow mb-auto">
          ...
          >
            {siteSettings?.accountMenu?.map(({ slug, name,icon }, index) => (
              <div key={index}  onClick={closeSidebar}>
                  ...
                  <div>
                    <Link
                      ...
                      className={twMerge(
                        cn(
                          'flex gap-2 w-full py-2.5 px-6 text-sm font-semibold capitalize text-heading transition duration-200 hover:text-white hover:bg-black focus:outline-0 ltr:text-left rtl:text-right',
                          'text-heading'
                        )
                      )}
                    >
