...
export default function Dashboard() {
  ...
  return (
    <div className="grid gap-7 md:gap-8 lg:grid-cols-2 2xl:grid-cols-12">
      ...
      <div className='bg-white col-span-full w-full rounded-xl p-4'>
        <h1 className='text-heading text-lg font-semibold mb-4'>{t("text-products-report")}</h1>
        <table className='table-auto w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400'>
          <thead className="text-xs text-gray-700 uppercase dark:text-gray-400">
            <th scope="col" className="px-6 py-3 bg-gray-50 dark:bg-gray-800">{t("text-product")}</th>
            <th scope="col" className="px-6 py-3">{t("text-total-orders-amount")}</th>
            <th scope="col" className="px-6 py-3 bg-gray-50 dark:bg-gray-800">{t("text-total-orders-number")}</th>
            <th scope="col" className="px-6 py-3">{t("text-total-orders-amount-discounted")}</th>
            <th scope="col" className="px-6 py-3 bg-gray-50 dark:bg-gray-800">{t("text-total-orders-number-discounted")}</th>
            <th scope="col" className="px-6 py-3 bg-gray-50 dark:bg-gray-800">{t("text-total-orders-amount-not-discounted")}</th>
            <th scope="col" className="px-6 py-3">{t("text-total-orders-number-not-discounted")}</th>

          </thead>
          <tbody>
            {data?.orderPorducts?.map((product:any) => (
              <tr className="border-b border-gray-200 dark:border-gray-700" key={product.id}>
                <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap bg-gray-50 dark:text-white dark:bg-gray-800">{product.name}</td>
                <td className="px-6 py-4">{Intl.NumberFormat().format(product.total_orders)}</td>
                <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap bg-gray-50 dark:text-white dark:bg-gray-800">{product.count_orders}</td>
                <td className="px-6 py-4">{Intl.NumberFormat().format(product.total_orders_discounted)}</td>
                <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap bg-gray-50 dark:text-white dark:bg-gray-800">{product.count_orders_dicounted}</td>
                <td className="px-6 py-4">{Intl.NumberFormat().format(product.total_orders_not_dicounted)}</td>
                <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap bg-gray-50 dark:text-white dark:bg-gray-800">{product.count_orders_not_discounted}</td>
                </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className='bg-white col-span-full w-full rounded-xl p-4'>
        <h1 className='text-heading text-lg font-semibold mb-4'>{t("text-customers-report")}</h1>
        <table className='table-auto w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400'>
          <thead className="text-xs text-gray-700 uppercase dark:text-gray-400">
            <th scope="col" className="px-6 py-3 bg-gray-50 dark:bg-gray-800">{t("text-customer")}</th>
            <th scope="col" className="px-6 py-3">{t("text-total-orders-amount")}</th>
            <th scope="col" className="px-6 py-3 bg-gray-50 dark:bg-gray-800">{t("text-total-orders-number")}</th>
            <th scope="col" className="px-6 py-3">{t("text-total-orders-amount-discounted")}</th>
            <th scope="col" className="px-6 py-3 bg-gray-50 dark:bg-gray-800">{t("text-total-orders-number-discounted")}</th>
            <th scope="col" className="px-6 py-3 bg-gray-50 dark:bg-gray-800">{t("text-total-orders-amount-not-discounted")}</th>
            <th scope="col" className="px-6 py-3">{t("text-total-orders-number-not-discounted")}</th>
            <th scope="col" className="px-6 py-3 bg-gray-50 dark:bg-gray-800">{t("text-total-commission")}</th>

          </thead>
          <tbody>
            {data?.orderCustomers?.map((product:any) => (
              <tr className="border-b border-gray-200 dark:border-gray-700" key={product.id}>
                <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap bg-gray-50 dark:text-white dark:bg-gray-800">{product.name}</td>
                <td className="px-6 py-4">{Intl.NumberFormat().format(product.total_orders)}</td>
                <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap bg-gray-50 dark:text-white dark:bg-gray-800">{product.count_orders}</td>
                ...
                <td className="px-6 py-4">{Intl.NumberFormat().format(product.total_orders_not_dicounted)}</td>
                <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap bg-gray-50 dark:text-white dark:bg-gray-800">{product.count_orders_not_discounted}</td>
                <td className="px-6 py-4">{Intl.NumberFormat().format(product.total_commission)}</td>
                </tr>
            ))}
          </tbody>
        </table>
