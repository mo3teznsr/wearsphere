...
class WithdrawController extends CoreController
{
    ...
    public function fetchWithdraws(Request $request)
    {
        try {
            ...
            if (!$user->hasPermissionTo(Permission::SUPER_ADMIN)) {
                return $this->repository->where('shop_id', $user->id);
                ...
                if ($user && $user->hasPermissionTo(Permission::SUPER_ADMIN)) {
                    return $this->repository->with(['customer'])->where('id', '!=', null);
                } else {
                    throw new AuthorizationException(NOT_AUTHORIZED);
                }
            }
    ...
    public function store(WithdrawRequest $request)
    {
        try {
            if ($request->user() 
            //&& ($request->user()->hasPermissionTo(Permission::SUPER_ADMIN) || $request->user()->shops->contains('id', $request->shop_id)
            ) {
                ...
                // }
                $id=$request->user()->id;
                $balance = Balance::where('shop_id', '=', $id)->first();
                ...
                $withdraw = $this->repository->create([...$request->all(), 'shop_id' => $id]);
    ...
    public function fetchSingleWithdraw(Request $request)
    {
        try {
            $id = $request->id;
            $withdraw = $this->repository->with(['customer'])->findOrFail($id);
