...
class AnalyticsController extends CoreController
{
    ...
    public function analytics(Request $request)
    {
        try {
            ...
            $orderPorducts=DB::select("select 
id,name,image,
(select sum(order_product.subtotal) from order_product join orders where orders.order_status='order-completed' and order_product.product_id=P.id ) as total_orders,
(select count(order_product.subtotal) from order_product join orders where orders.order_status='order-completed' and order_product.product_id=P.id ) as count_orders,
(select sum(order_product.subtotal) from order_product join orders where orders.order_status='order-completed' and orders.discount>0 and order_product.product_id=P.id ) as total_orders_discounted,
(select count(order_product.subtotal) from order_product join orders where orders.order_status='order-completed' and orders.discount>0 and order_product.product_id=P.id ) as count_orders_dicounted,
(select sum(order_product.subtotal) from order_product join orders where orders.order_status='order-completed' and orders.discount=0 and order_product.product_id=P.id ) as total_orders_not_dicounted,
(select count(order_product.subtotal) from order_product join orders where orders.order_status='order-completed' and orders.discount=0 and order_product.product_id=P.id ) as count_orders_not_discounted
from products as P order by total_orders desc limit 10;");
            $orderCustomers=DB::select("select 
id,name,
(select sum(orders.total) from orders where orders.order_status='order-completed' and orders.customer_id=P.id ) as total_orders,
(select count(orders.total) from orders where orders.order_status='order-completed' and orders.customer_id=P.id ) as count_orders,
(select sum(orders.total) from orders where orders.order_status='order-completed' and orders.customer_id=P.id and orders.discount>0) as total_orders_discounted,
(select count(orders.total) from orders where orders.order_status='order-completed' and orders.customer_id=P.id and orders.discount>0 ) as count_orders_dicounted,
(select sum(orders.total) from orders where orders.order_status='order-completed' and orders.customer_id=P.id and orders.discount=0) as total_orders_not_dicounted,
(select count(orders.total) from orders where orders.order_status='order-completed' and orders.customer_id=P.id and orders.discount=0 ) as count_orders_not_discounted,
(select sum(orders.total_commission) from orders where orders.order_status='order-completed' and orders.customer_id=P.id) as total_commission
from users as P 
