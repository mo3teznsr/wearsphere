...
class OrderController extends CoreController
{
    ...
    public function fetchOrders(Request $request)
    {
        ...
        if ($request->has('dateFrom')&&isset($request->dateFrom)) {
            $query->whereDate('created_at', '>=', $request->dateFrom);
        }
    
        if ($request->has('dateTo')&&isset($request->dateTo)) {
        ...
        if ($user->hasPermissionTo(Permission::SUPER_ADMIN)) {
            $query->where('id', '!=', null)->where('parent_id', '=', null);
        } elseif ($user->hasPermissionTo(Permission::STORE_OWNER)) {
            if ($this->repository->hasPermission($user, $request->shop_id)) {
                $query->where('shop_id', '=', $request->shop_id)->where('parent_id', '!=', null);
            } else {
                $query->where('parent_id', '!=', null)->whereIn('shop_id', $user->shops->pluck('id'));
            }
        } elseif ($user->hasPermissionTo(Permission::STAFF)) {
            if ($this->repository->hasPermission($user, $request->shop_id)) {
                $query->where('shop_id', '=', $request->shop_id)->where('parent_id', '!=', null);
            } else {
                $query->where('parent_id', '!=', null)->where('shop_id', '=', $user->shop_id);
            }
        } else {
            $query->where('customer_id', '=', $user->id)->where('parent_id', '=', null);
        }
...
FROM 
    (SELECT 'order-processing' AS status
     UNION ALL
     SELECT 'order-completed'
     UNION ALL
     SELECT 'order-cancelled'
     UNION ALL
     SELECT 'order-refunded'
     UNION ALL
     SELECT 'order-failed'
     UNION ALL
     SELECT 'order-at-local-facility'
     UNION ALL
     SELECT 'order-out-for-delivery'
     UNION ALL
     SELECT 'order-pending') s
LEFT JOIN 
    orders o ON s.status = o.order_status
GROUP BY 
    s.status where customer_id = 8");
    }
