...
class OrderRepository extends BaseRepository
{
    ...
    public function storeOrder($request, $settings): mixed
    {
        ...
        try {
            ...
            if ($user) {
                $request['customer_name'] = $request['name']??$user->name;
            }
        } catch (Exception $e) {
            $user = null;
        }
    ...
    protected function calculateShopIncome($parent_order)
    {
        foreach ($parent_order->children as  $order) {
            $balance = Balance::where('shop_id', '=', $request->user()->id)->first();
            if(!isset($balance)){
              $balance = Balance::create(['shop_id' => $request->user()->id, 'total_earnings' => 0, 'current_balance' => 0]);
            }
            $adminCommissionRate = $balance->admin_commission_rate;
            $shop_earnings = $parent_order->total_commission;
